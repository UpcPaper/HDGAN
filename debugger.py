...
class DebuggerBase:
    def __init__(self, args):
        ...
        self.writer = self._init_writer()  # 自己定义的  def
    ...
    def _init_train_transform(self):  # Transfoms是很常用的图片变换方式，通过compose将各个变换串联起来
        transform = transforms.Compose([  # 这个类将多个变换方式结合在一起   参数：各个变换的实例对象
    ...
    def _init_model_dir(self):
        model_dir = os.path.join(self.args.model_path, self.args.saved_model_name)   # 路径拼接文件路径

        if not os.path.exists(model_dir):
            os.makedirs(model_dir)  # 递归创建目录
    ...
    def _init_model_path(self):  # 模型路径
