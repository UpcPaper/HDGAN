...
class CaptionSampler(object):
    def __init__(self, args):
        ...
        self.data_loader = self.__init_data_loader(self.args.file_lits) # 随机生成 the path for test file list
    ...
    def __save_json(self, result):
        ...
        with open(os.path.join(result_path, '{}.json'.format(self.args.load_model_path+self.args.result_name)), 'w') as f:
            json.dump(result, f)  # 将json信息写进文件  dump

    def __load_mode_state_dict(self):
        try:
            model_state_dict = torch.load(os.path.join(self.args.model_dir, self.args.load_model_path))#改
            print("[Load Model-{} Succeed!]".format(self.args.load_model_path))  # train_best_loss.pth.tar 改
...
if __name__ == '__main__':
    ...
    """
    # Path Argument
    parser.add_argument('--model_dir', type=str, default='./report_v4_models/v4/lstm')  #   20190829-13:39/  ./report_v4_models/v4/20190802-07:33/  ./report_v4_models/v4/20190724-02:44/
...
for file_name in os.listdir(args.model_dir):
    ...
    if (len(name) > 1 and name[-1] == 'tar'):

        parser.set_defaults(load_model_path=file_name)
        args = parser.parse_args()
        ...
        for i, line in enumerate(lines):
            if i % 3 == 1:
                ...
                lines[i] = str(lines[i]).replace('0:', '').replace('1:', '').replace('2:', '').replace('3:',
                                                                                                       '').replace('4:',
                                                                                                                   '').replace(
